...
import torch
from mmcv.runner import AdvRunner, DistSamplerSeedHook
...
def parse_losses(losses):
    ...
    loss = sum(_value for _key, _value in log_vars.items() if 'loss_cls' in _key)
    ...
    if validate:
        if cfg.data.val.type in ['RawFramesDataset', 'VideoDataset']:
            runner.register_hook(
                DistEvalTopKAccuracyHook(cfg.data.val, k=(1, 5), interval=1))
